...
const uploadRouter = express.Router(); // Router for legacy upload endpoint
const upload = multer();

// Admin: POST /api/admin/product-image - add product image record (client already uploaded to storage)
adminRouter.post("/", async (req, res) => {
  try {
    ...
    if (!product_id || !image_url) {
      return sendError(res, "product_id (or productId) and image_url are required", 400);
    }

    // Validate product exists
    const productCheck = await pool.query(`SELECT id FROM product WHERE id = $1 LIMIT 1`, [product_id]);
    ...
    const { rows } = await pool.query(q, [imageId, product_id, image_url, sort_order]);
...
// Admin: POST /api/admin/product/:id/image - upload image to Supabase Storage (legacy - use POST /api/admin/product-image instead)
uploadRouter.post("/:id/image", upload.single("file"), async (req, res) => {
  try {
...
adminRouter.delete("/:imageId", async (req, res) => {
  try {
...
adminRouter.put("/:imageId/sort", async (req, res) => {
  try {
    const { imageId } = req.params;
